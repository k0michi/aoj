...
int main() {
  ...
  List list;
  constructList(&list, sizeof(i32), 0);
  ...
  for (i32 i = 0; i < n; i++) {
    ...
    if (strcmp(command, "insert") == 0) {
      ...
      i32 x;
      scanf("%" SCNi32, &x);
      ListIterator it = listFind(&list, &x, i32Equals);

      if (!listIteratorEquals(it, listEnd(&list))) {
        listErase(&list, &it);
      }
  ...
  for (ListIterator it = listBegin(&list);
       !listIteratorEquals(it, listEnd(&list)); listIteratorIncrement(&it)) {
    if (!listIteratorEquals(it, listBegin(&list))) {
      printf(" ");
    }

    printf("%" PRIi32, listIteratorGetAs(&it, i32));
  }
