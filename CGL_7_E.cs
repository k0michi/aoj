...
struct Circle {
  ...
  public Circle(Vector2 origin, double radius) {
    Origin = origin;
    Radius = radius;
  }
}
...
static class MathHelper {
  ...
  public static double Arg(Vector2 v) {
    return Math.Atan2(v.Y, v.X);
  }

  public static Vector2 Polar(double rho, double theta) {
    return new Vector2(Math.Cos(theta) * rho, Math.Sin(theta) * rho);
  }

  public static List<Vector2> CrossPointsCircles(Circle c1, Circle c2) {
    double d = Length(c1.Origin - c2.Origin);
    double a = Math.Acos((c1.Radius * c1.Radius + d * d - c2.Radius * c2.Radius) / (2 * c1.Radius * d));
    double t = Arg(c2.Origin - c1.Origin);
    List<Vector2> result = new List<Vector2>{
      c1.Origin + Polar(c1.Radius , t + a) , c1.Origin + Polar(c1.Radius, t - a)
    };

    result.Sort((Vector2 v1, Vector2 v2) => {
      if (v1.X != v2.X) {
...
class Program {
  static void Main(string[] args) {
    Circle c1 = new Circle(new Vector2(ConsoleScanner.NextInt32(), ConsoleScanner.NextInt32()), ConsoleScanner.NextInt32());
    Circle c2 = new Circle(new Vector2(ConsoleScanner.NextInt32(), ConsoleScanner.NextInt32()), ConsoleScanner.NextInt32());
    var result = MathHelper.CrossPointsCircles(c1, c2);

    Console.WriteLine($"{result[0].X:f6} {result[0].Y:f6} {result[1].X:f6} {result[1].Y:f6}");
  }
}