...
i32 bubbleSort(Vector *seq) {
  ...
  while (flag) {
    ...
    for (i32 j = seq->length - 1; j >= i + 1; j--) {
      if (vectorGetAs(seq, i32, j) < vectorGetAs(seq, i32, j - 1)) {
        swap(vectorGetAddressAs(seq, i32, j),
             vectorGetAddressAs(seq, i32, j - 1));
        flag = true;
        swapCount++;
      }
    }
