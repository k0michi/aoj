...
void bubbleSort(Vector *a, usize size,
                bool (*compare)(const void *, const void *)) {
  bool flag = true;

  for (usize i = 0; flag; i++) {
    flag = false;

    for (usize j = vectorSize(a) - 1; j >= i + 1; j--) {
      if (compare(vectorGetAddress(a, j), vectorGetAddress(a, j - 1))) {
        swapVoid(vectorGetAddress(a, j), vectorGetAddress(a, j - 1),
                 vectorElementSize(a));
        flag = true;
      }
    }
...
void selectionSort(Vector *a, usize size,
                   bool (*compare)(const void *, const void *)) {

  for (i32 i = 0; i < vectorSize(a); i++) {
    i32 minJ = i;

    for (i32 j = i; j < vectorSize(a); j++) {
      if (compare(vectorGetAddress(a, j), vectorGetAddress(a, minJ))) {
        minJ = j;
      }
    }

    if (i != minJ) {
      swapVoid(vectorGetAddress(a, i), vectorGetAddress(a, minJ),
               vectorElementSize(a));
    }
  }
}

bool compareCard(const void *a, const void *b) {
...
int main() {
  ...
  Vector cards;
  constructVectorOf(&cards, Card, n);

  for (i32 i = 0; i < n; i++) {
    Card card;
    scanf(" %c%" SCNd32, &card.suit, &card.value);
    vectorSet(&cards, i, &card);
  }

  Vector cardsBubble;
  constructVectorOf(&cardsBubble, Card, n);
  vectorCopy(&cardsBubble, &cards);
  bubbleSort(&cardsBubble, n, compareCard);
  ...
  for (i32 i = 0; i < n; i++) {
    ...
    Card card = vectorGetAs(&cardsBubble, Card, i);
    printf("%c%" PRId32, card.suit, card.value);
  }

  printf("\n");
  printf("Stable\n");

  Vector cardsSelection;
  constructVectorOf(&cardsSelection, Card, n);
  vectorCopy(&cardsSelection, &cards);
  selectionSort(&cardsSelection, n, compareCard);
  ...
  for (i32 i = 0; i < n; i++) {
    ...
    Card card = vectorGetAs(&cardsSelection, Card, i);
    printf("%c%" PRId32, card.suit, card.value);
  }

  printf("\n");

  if (vectorEquals(&cardsBubble, &cardsSelection)) {
    printf("Stable\n");
  } else {
    printf("Not stable\n");
  }
