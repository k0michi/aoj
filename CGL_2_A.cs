...
class Program {
  ...
  static void Main(string[] args) {
    ...
    for (int i = 0; i < q; ++i) {
      ...
      Vector2 s1 = MathHelper.Normalize(p1 - p0);
      Vector2 s2 = MathHelper.Normalize(p3 - p2);

      double dot = MathHelper.Dot(s1, s2);

      if (Math.Abs(dot) < kEps) {
        Console.WriteLine(1);
      } else if (Math.Abs(dot - 1) < kEps || Math.Abs(dot - (-1)) < kEps) {
        Console.WriteLine(2);
      } else {
        Console.WriteLine(0);
      }
