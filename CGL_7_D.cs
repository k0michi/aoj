...
class Program {
  static List<Vector2> CrossPointCircleLine(Circle c, Segment2 l) {
    Vector2 mid = MathHelper.Project(l.P0, l.P1, c.Origin);
    Vector2 lv = MathHelper.Normalize(l.P1 - l.P0);
    double h = Math.Sqrt(c.Radius * c.Radius - MathHelper.LengthSquared(mid - c.Origin));
    List<Vector2> result = new List<Vector2> { mid + lv * h, mid - lv * h };

    result.Sort((Vector2 a, Vector2 b) => {
      if (a.X != b.X) {
  ...
  static void Main(string[] args) {
    Circle circle = new Circle(new Vector2(ConsoleScanner.NextInt32(), ConsoleScanner.NextInt32()), ConsoleScanner.NextInt32());
    int q = ConsoleScanner.NextInt32();

    for (int i = 0; i < q; ++i) {
      Segment2 line = new Segment2(new Vector2(ConsoleScanner.NextInt32(), ConsoleScanner.NextInt32()), new Vector2(ConsoleScanner.NextInt32(), ConsoleScanner.NextInt32()));

      List<Vector2> result = CrossPointCircleLine(circle, line);
